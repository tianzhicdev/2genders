...
def get_db_connection():
    conn = psycopg2.connect(
        host=db_config['remote']['db_host'],
        port=db_config['remote']['db_port'],
        dbname=db_config['remote']['db_name'],
...
@rest_api.route('/api/profile', methods=['POST'])
class CreateProfile(Resource):
    """
       Creates a new profile by taking JSON input
    """
    ...
    def post(self):
        ...
        try:
            ...
            cur = conn.cursor()
            cur.execute(
                "INSERT INTO profile (data) VALUES (%s) RETURNING key;",
                (Json(data),)
            )
            ...
            cur.close()
            conn.close()
            return {"profile_id": profile_id}, 201
